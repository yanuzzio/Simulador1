    ...
    public void volteador_Abajo()
    {
        if(Proceso.canReproduceAnimation)
        {
                ...
                && ElementoSP_Colliders.elementoEnContenedor && BaseLP_Colliders.reactorVolteadorAbajo)
            {
                ...
                mainAnimatorController.Desplazamiento_Ida(); //Función para disparar el respectivo trigger de la animación
            }
            else if(!Tutorial.canDoTutorial)
            {
                //Condición contraria del botón "Posicio_Carro_Vuelta"
                ...
                else if (BaseLP_Colliders.reactorVolteadorAbajo && Proceso.marchaIda)
                {
                    ...
                    mainAnimatorController.SE_Desplazamiento_Ida(); //Función para disparar el respectivo trigger de la animación
                }
                #endregion
